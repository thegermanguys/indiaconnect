# IndiaConnect Germany 🇮🇳🇩🇪

**The Home of Indians in Germany.**

The largest online platform connecting Indians across Germany — sports clubs, communities,
...
- **Color** — the national flag's saffron (`--saffron`) and india-green (`--india-green`) anchor
  the palette, with an Ashoka-Chakra navy (`--navy`) for headers/dark sections and a marigold
  accent (`--marigold`, used in Diwali/Navratri contexts).
...
- **Signature element** — a three-colour `.tricolor-strip` (in `globals.css`), based on the
  flag's saffron/white/green band order, used as the section divider throughout the site instead
  of a plain hairline rule. The navbar/footer mark (`components/shared/logo-mark.tsx`) is a
  self-contained SVG — a tricolor disc with a simplified Ashoka Chakra — so no binary logo asset
  is required.
...
5. **Seed data — real clubs & restaurants, illustrative events** — `npm install`, then `npm run
   seed`. This loads `supabase/seed-data/*.json`: 69 German cities, 12 categories, and:
   - **25 clubs/associations** — real, publicly documented organizations found via web search
     (cricket clubs like Munich Cricket Club and Berlin Cricket Club, registered associations like
     German Tamil Sangam e.V. and Mana Telugu Association Germany, Hindu temples and gurudwaras,
     and university Indian student associations like AISA Aachen and MISA Munich). Only verified
     public facts are included — no invented contact names, emails, or phone numbers for these
     real organizations.
   - **12 restaurants** — real, named Indian restaurants with verified addresses (Amrit and
     Maharadscha in Berlin, Saravanaa Bhavan's branches, Prince of India in Stuttgart, Mayur in
     Düsseldorf, etc.), sourced from restaurant review sites and each restaurant's own site.
   - **10 events** — these are **illustrative placeholders only** (fictional dates/venues for
     recurring festivals like Diwali, Holi, and Navratri) since real upcoming event listings
   ...
   associations and temples — reach out to let them know they're listed and invite them to claim
